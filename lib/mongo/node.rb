  ...
  class Node
    ...
    def initialize(connection, data)
      ...
      if data.is_a?(String)
        ...
        self.host = data[0]
        self.port = data[1].nil? ? Connection::DEFAULT_PORT : (data[1] == :socket || data[1] == 'socket' ? :socket : data[1].to_i)
      end
      self.address = port == :socket ? "#{host}" : "#{host}:#{port}"
    end
      ...
      begin
        ...
        if self.connection.connect_timeout
          Mongo::TimeoutHandler.timeout(self.connection.connect_timeout, OperationTimeout) do
            socket = unless unix_domain_socket?
                       TCPSocket.new(self.host, self.port)
                     else
                       UNIXSocket.new(self.host)
                     end
          end
        else
          socket = unless unix_domain_socket?
    ...
    def split_nodes(host_string)
      ...
      host = data[0]
      port = data[1].nil? ? Connection::DEFAULT_PORT : (data[1] == 'socket' ? :socket : data[1].to_i)
